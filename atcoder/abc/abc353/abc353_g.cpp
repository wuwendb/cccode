...
void solve() {  
    int n,c;cin>>n>>c;
    int m;cin>>m;
    SegmentTree<Info> seg1(n),seg2(n);
    seg1.modify(0,{0});
    seg2.modify(0,{0});
    i64 ans = 0;
    rep(i,0,m-1){
    	int t;i64 p;cin>>t>>p;t--;
    	i64 res = max(seg1.rangeQuery(0,t).ma-1LL*c*t,seg2.rangeQuery(t,n).ma+1LL*c*t)+p;
    	ans = max(ans,res);
    	seg1.modify(t,{res+1LL*c*t});
    	seg2.modify(t,{res-1LL*c*t});
    }
    cout<<ans<<endl;
