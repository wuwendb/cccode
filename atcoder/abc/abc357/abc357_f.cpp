...
struct Info {
    ...
    void apply(const Tag &t) {
        sum[0][0] += sum[0][1] * t.addy + sum[1][0] * t.addx + sum[1][1] * t.addx * t.addy;
        sum[0][1] += sum[1][1] * t.addx;
        sum[1][0] += sum[1][1] * t.addy;
    }
};
...
void solve() {  
    ...
    rep(i,0,n-1){
    	seg.modify(i,{{{a[i]*b[i],a[i]},{b[i],1}}});
    }
    rep(i,1,q){
    	int op;cin>>op;
    	if(op==1){
    		int l,r,x;cin>>l>>r>>x;l--;
    		seg.rangeApply(l,r,{x,0});
    	}else if(op==2){
    		int l,r,x;cin>>l>>r>>x;l--;
    		seg.rangeApply(l,r,{0,x});
    	}else{
    		int l,r;cin>>l>>r;l--;
    		cout<<seg.rangeQuery(l,r).sum[0][0]<<endl;
    	}
    }
