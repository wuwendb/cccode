// Problem: G - Add and Multiply Queries
...
Info operator+(const Info &i1, const Info &i2) {
    return {i1.a+i2.a,max(i1.b,i2.b)};
}
...
void solve() {  
    ...
    while(q--){
    	...
    	if(op==1){
    		...
    		int v = 0;
    		while(l<r){
    			auto nx = seg.findFirst(l,r,[&](auto& p){
    				return p.b>1;
    			});
    			if(nx==-1){
    				v+=seg.rangeQuery(l,r).a;
    				break;
				}
				v+=seg.rangeQuery(l,nx).a;
				auto [a,b] = seg.rangeQuery(nx,nx+1);
				l=nx+1;
				v=max(v+a,v*b);
    		}
    		cout<<v<<endl;
